...
- **REST API как инструмент AI-агента** — AI-агент использует стандартные HTTP-вызовы (`POST /device-tasks/`, `GET /device-events/`) для управления устройствами и получения телеметрии, что делает платформу совместимой с протоколами tool-use (Function Calling, MCP)
...
flowchart LR
    ...
    subgraph Devices ["IoT-устройства"]
        D1["ESP32<br/>Замки, датчики"]
        D2["STM32<br/>Периферия"]
        D3["Python Agent<br/>Камеры, автоматизация"]
    end
...
#### 4. Мультиустройственная оркестрация

AI-агент координирует действия нескольких устройств в рамках единого сценария, используя последовательные или параллельные вызовы API.

```
AI-агент → POST /device-tasks/ (устройство A: считать датчик)
         → POST /device-tasks/ (устройство B: считать датчик)
         → Анализ данных обоих устройств
         → POST /device-tasks/ (устройство C: выполнить действие)
```
