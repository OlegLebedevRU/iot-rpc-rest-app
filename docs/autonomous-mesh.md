...
> Автономная отказоустойчивая архитектура Leo4 для замкнутых контуров управления: **3–5 идентичных ARM64-копий платформы** с **реактивным оркестратором без состояния** на каждой копии, единым VIP и переключением устройств между портами при отказах.
...
1. Автономная замкнутая сеть из **N идентичных копий платформы** на ARM64-платах (например, Raspberry Pi 4/5), где на практике обычно используется диапазон **3–5 узлов**. Такой диапазон даёт баланс между резервированием и простотой эксплуатации локального контура.
...
7. На каждой копии развёрнут **реактивный оркестратор без состояния**: он не опирается на накопленное состояние и реагирует только на входящие события по фиксированному алгоритму.
...
flowchart TB
    ...
    subgraph MESH["Автономная сеть платформы (замкнутый контур, единый VIP)"]
        ...
        subgraph N1["Копия платформы #1 (порт 8883)"]
            ...
            O1["Реактивный оркестратор<br/>(без состояния)"]
            NG1 --> C1
            B1 <--> C1
            C1 --> DB1
        ...
        subgraph N3["Копия платформы #3..N (порты 8885, 8886, ...)"]
            NG3["nginx ARM64"]
            C3["Core app"]
            B3["MQTT-брокер"]
            DB3["Локальная БД"]
...
Оркестратор в Leo4 Autonomous Mesh — это реактивный компонент без состояния, развёрнутый на каждой копии платформы. Он подписывается на события устройств и, по фиксированным правилам, публикует идемпотентные RPC-команды.
...
Ожидаемое время переподключения определяется политикой самого устройства и параметрами локальной сети. Обычно речь идёт о диапазоне от секунд до десятков секунд, но целевые значения RTO (Recovery Time Objective, целевое время восстановления) фиксируются в эксплуатационном регламенте конкретного объекта. В рамках данного концептуального документа конкретные тайминги не нормируются.

---

## 🔐 Безопасность в автономном контуре

В варианте Autonomous Mesh безопасность строится на сочетании закрытого сетевого периметра и PKI/mTLS:

- доверие к устройствам и узлам задаётся сертификатами внутри локального контура;
- доступ ограничивается сетевой сегментацией и правилами периметра;
- отсутствие JWT-контура в этой модели компенсируется тем, что контур изначально изолирован и не рассчитан на внешний публичный доступ.
