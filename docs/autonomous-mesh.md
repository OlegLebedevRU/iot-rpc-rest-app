...
> Автономная отказоустойчивая архитектура Leo4 для замкнутых контуров управления: **3–5 идентичных ARM64-копий платформы** с **реактивным оркестратором без состояния (stateless)** на каждой копии, единым VIP и переключением устройств между портами при отказах.
...
1. Автономная замкнутая сеть из **3–5 идентичных копий платформы** на ARM64-платах (например, Raspberry Pi 4/5), где `N` — фактическое количество узлов в этом диапазоне. Диапазон выбран как практичный баланс между резервированием и простотой эксплуатации: обычно этого достаточно, чтобы переживать отказ части узлов без усложнения локального контура.
...
flowchart TB
    ...
    subgraph MESH["Автономная сеть платформы (замкнутый контур, единый VIP)"]
        ...
        subgraph N3["Копия платформы #3..N (порты 8885..888N)"]
            NG3["nginx ARM64"]
            C3["Core app"]
            B3["MQTT-брокер"]
            DB3["Локальная БД"]
...
Ожидаемое время переподключения определяется политикой самого устройства и параметрами локальной сети. Целевые значения RTO фиксируются в эксплуатационном регламенте конкретного объекта. В рамках данного концептуального документа конкретные тайминги не нормируются.
