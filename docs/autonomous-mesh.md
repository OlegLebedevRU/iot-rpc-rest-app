...
1. Автономная замкнутая сеть из **3–5 (N) идентичных копий** платформы на ARM64-платах (например, Raspberry Pi 4/5). Диапазон выбран как практичный баланс между резервированием и простотой эксплуатации локального контура.
...
Ожидаемое время переподключения определяется политикой самого устройства и параметрами локальной сети; в рамках данного концептуального документа конкретные тайминги не нормируются.
