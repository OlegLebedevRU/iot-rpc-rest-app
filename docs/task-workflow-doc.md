...
>- Основной функционал — удаленная инициация команд, загрузка данных и управления задачами на IoT-устройствах.
>- Для приложений и веб-сервисов реализован протокол HTTPS+JSON.
>- Для взаимодействие между приложениями используются токены доступа JWT (или постоянные токены api-key).
>- Под капотом АПИ — системная шина и очереди с приоритезацией, а также механизмом TTL.
>- Для обмена на нижнем уровне (с устройствами) используется защищенный (mutual TLS) асинхронный RPC-протокол поверх MQTT v5.
...
- Дублированная стратегия протокола (push & pull) позволяет управлять устройствами даже в условиях прерывания и нестабильности сети
- TTL устанавливает срок жизни команды, позволяя удерживать в очереди задачи загрузки конфигураций и данных для отложенной загрузки и обратно - автоматически отменять в очереди срочные команды для отключенных устройств
- Приоритезация (priority) дает возможность обойти очередь и поставить в топ срочную команду.
