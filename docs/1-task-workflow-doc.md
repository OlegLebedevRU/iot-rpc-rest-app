...
````
>- Суффикс URL хука `/fddf6675-42d3-....`,  соответствует ID задания. 
>- Заголовок `X-Status-Code: 200` по аналогии с заголовком ответа HTTP `StatusCode` прокидывается от устройства.
>- Реультаты выполнения задачи могут содержать несколько элементов, которые доставляются отдельными хуками.
...
> Для `method_code=51` именно событие `CellOpenEvent`, где `tag=304`, а значение события равно `5`, подтверждает, что ячейка действительно открылась.
