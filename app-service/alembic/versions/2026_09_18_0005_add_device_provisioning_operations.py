...
Revision ID: 0005_device_provisioning
...
revision: str = "0005_device_provisioning"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(length=32), server_default="requested", nullable=False
        ),
...
def downgrade() -> None:
    op.drop_index(
        "ix_tb_device_provisionings_tenant_terminal",
        table_name="tb_device_provisionings",
    )
    op.drop_index(
        "ix_tb_device_provisionings_status", table_name="tb_device_provisionings"
    )
    op.drop_index(
        "ix_tb_device_provisionings_device_id", table_name="tb_device_provisionings"
    )
    op.drop_index("ix_tb_device_provisionings_sn", table_name="tb_device_provisionings")
    op.drop_index(
        "ix_tb_device_provisionings_terminal_id", table_name="tb_device_provisionings"
    )
    op.drop_index(
        "ix_tb_device_provisionings_tenant_id", table_name="tb_device_provisionings"
    )
    op.drop_index(
        "ix_tb_device_provisionings_operation_id", table_name="tb_device_provisionings"
    )
