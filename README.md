...
| 🕸️ [**Leo4 Autonomous Mesh**](./docs/autonomous-mesh.md) | Автономная отказоустойчивая сеть из N идентичных ARM64-копий платформы с реактивным stateless-оркестратором |
...
> Автономная архитектура для замкнутых контуров: несколько одинаковых ARM64-копий Leo4, единый VIP-адрес с разными портами, реактивный stateless-оркестратор на каждой копии и идемпотентный RPC на устройствах.
...
flowchart TB
    ...
    subgraph MESH["Leo4 Autonomous Mesh"]
        direction LR
        N1["Копия #1<br/>nginx ARM64, Core app,<br/>MQTT-брокер, локальная БД,<br/>stateless-оркестратор"]
        N2["Копия #2<br/>nginx ARM64, Core app,<br/>MQTT-брокер, локальная БД,<br/>stateless-оркестратор"]
        N3["Копия #3..N<br/>nginx ARM64, Core app,<br/>MQTT-брокер, локальная БД,<br/>stateless-оркестратор"]
    end
