...
| 🕸️ [**Leo4 Autonomous Mesh**](./docs/autonomous-mesh.md) | Автономная отказоустойчивая сеть из N идентичных ARM64-копий платформы с реактивным оркестратором без состояния (stateless) |
...
> Автономная архитектура для замкнутых контуров: несколько одинаковых ARM64-копий Leo4, единый VIP-адрес с разными портами, реактивный оркестратор без состояния (stateless) на каждой копии и идемпотентный RPC на устройствах.
...
Модель рассчитана на изолированный контур с mTLS и закрытым периметром доверия: при отказе от JWT безопасность обеспечивается PKI-подходом и сетевой сегментацией контура.
