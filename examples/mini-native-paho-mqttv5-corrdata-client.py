...
def on_message(client, userdata, msg):
    ...
    if hasattr(msg.properties, "CorrelationData"):
        print(
            "Native msg.properties.CorrelationData = "
            + str(msg.properties.CorrelationData.decode())
        )
    ...
    if msg.topic.endswith("tsk"):
        ...
        if hasattr(msg.properties, "CorrelationData"):
            props.UserProperty = [
                ("CorrelationData", str(msg.properties.CorrelationData.decode()))
            ]
                ...
                json.dumps(
                    {
                        ...
                        "corr_data": f"{str(msg.properties.CorrelationData.decode())}",
                    }
                ),
                qos=0,
                properties=props,
...
with open(
    "D:/ol-factory/t0000000/cert_0000000.pem", "rb"
) as pem_file:  # sys.argv[1], 'rb'
...
print(cert["CN"])
mqttc = mqtt.Client(
    mqtt.CallbackAPIVersion.VERSION2, protocol=mqtt.MQTTv5, client_id=cert["CN"]
)  # "a3b0000000c10221d290825")
# mqttc.username_pw_set('user', 'root')
mqttc.on_message = on_message
mqttc.on_connect = on_connect
mqttc.tls_set(
    "D:/work/iot.leo4.ru/iot_leo4_ca.crt",
...
while True:
    ...
    props.CorrelationData = uuid.UUID(int=1).bytes
    props.UserProperty = [
        ("event_type_code", "90"),
    ...
    props.UserProperty = [("CorrelationData", str(uuid.UUID(int=0).bytes.decode()))]
    mqttc.publish("dev/" + cert["CN"] + "/req", f"req = {i}", qos=0, properties=props)
    props.clear()
    did = 10000 + i
    ...
    mqttc.publish(
        "dev/" + cert["CN"] + "/evt", json.dumps(gauge), qos=0, properties=props
    )
